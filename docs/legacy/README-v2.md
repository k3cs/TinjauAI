...
## Why a credit bureau
Creditcoin began as a credit history for borrowers that banks cannot see: its Credal API reports 5M+ loan transactions and 337k users (creditcoin.org). AI agents are the next borrowers with no readable history. Over 19,000 are registered on ERC-8004, they are hired and paid by other agents and contracts, and nothing a contract can check says which ones deserve trust. Tinjau does for agents what a credit bureau does for people. It records proven facts about each agent and the reviewers vouching for it, and lets the party taking the risk price it. Tinjau is not a lender and publishes no score. The hiring escrow is the pricing step: the premium it charges is the agent's cost of credit.
...
## Solution
Tinjau is a Creditcoin contract that admits **facts** about agents and their reviewers only through Attestcoin proofs of Ethereum transactions: which reviews exist, how long each reviewer was active before reviewing, whether review indices have gaps, whether a reviewer owns agents, and how many agents share an owner, registrant, URI or minting transaction. It computes no score. You pass your thresholds; it returns numbers anyone can recompute from the same proofs. An escrow turns the facts into a hiring premium (the bureau's price of trust), a bounty pays whoever brings evidence that changes a decision, and an autonomous scout decides what to prove.
