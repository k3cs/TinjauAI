...
**Framing (v1.5): biro kredit untuk agent AI.** [Fakta] Creditcoin lahir sebagai riwayat kredit untuk peminjam yang tidak terlihat bank (Credal: 5 jt+ transaksi pinjaman, 337 rb pengguna, adopter pertama Aella di Nigeria; creditcoin.org, dibaca 11 Sep 2026). [Inferensi] Agent AI adalah "peminjam" berikutnya tanpa riwayat yang bisa dicek kontrak. Tinjau memegang peran biro (mencatat fakta yang dibuktikan), bukan pemberi pinjaman (tidak menilai, tidak meminjamkan). Premi `AgentHireEscrow` adalah langkah penetapan harga: biaya kredit si agent.
...
[Fakta, dikoreksi 11 Sep] Versi 1.4 menulis "tidak ada repo peserta yang memakai mainnet". Klaim itu **salah**. Dari 87 BUIDL di galeri DoraHacks (dibaca 11 Sep via API), minimal 7 memakai Ethereum mainnet (chainKey 3), antara lain nomen, Singleton, index41, crosscredit, CarryProof, WEN, dan Collateral Eligibility Ledger. Singleton juga memakai tiga precompile (termasuk AttestorStash `0x0FD4`), yang tidak dipakai Tinjau.

[Inferensi] Yang khas Tinjau bukan pemakaian mainnet, melainkan: (a) subjeknya agent AI dan pengulasnya (0 dari 87 BUIDL menyentuh ERC-8004); (b) riwayat berumur tahunan dipakai sebagai input fakta (senioritas pengulas); (c) kelengkapan parsial dari counter protokol sumber (`feedbackIndex`); (d) dua registri × empat jenis event.
...
[Fakta, 11 Sep] Galeri DoraHacks memuat 87 BUIDL. 0 menyebut ERC-8004, registri agent, atau reputasi agent AI. Sekitar 22 adalah "credit passport" (skor kredit wallet peminjam dari riwayat pinjaman). Penilaian ide ke-87 BUIDL dengan rubrik yang sama ada di workspace (`docs/outputs/02-ideation/2026-09-11-penilaian-ide-87-buidl.md`, di luar repo ini).
...
### 8.5 Pengukuran lokal sebelum deploy (forge, precompile di-mock, 29 Agu)
...
Belum diukur (per 29 Agu; `sameTxSiblings` untuk 10 log sudah diukur live di §8.4): biaya cakupan penuh 14–27 pengulas di testnet nyata; tx registrasi massal >100 log; biaya `Registered` massal untuk pemilik 1.300 agent.
...
Deadline: diperpanjang ke 13 Sep 2026 23:59 ET (14 Sep 10:59 WIB); pengumuman pemenang 20 Sep 2026 (halaman DoraHacks, dibaca 11 Sep). Repo baru dibuat setelah 13 Agu sesuai aturan "original work".
...
| 7 | Video ≤3 menit | skrip siap (`docs/demo-script.md` dengan hash); direkam Dien setelah frontend final |

Demo ≤3 menit, semuanya data mainnet nyata yang sudah on-chain (rencana awal memakai agent 34135; diganti 22771 setelah deploy v2): (1) UI menampilkan agent **22771** (3 pengulas senior, premi 100 bps, `hire` berhasil) dan agent **50283** (1 pengulas yang memiliki 6 agent terbukti on-chain, 5 saudara klon terbukti, quote 1.683 bps, `hire` revert `Gated(1)`); (2) tx `record` di Creditcoin disandingkan dengan tx aslinya di Ethereum, termasuk tx Jan 2024; (3) log scout: target dipilih sendiri, bukti dua arah, bounty diklaim, sewa (R4), siklus kedua 0 gas; (4) `verify.ts` menghitung ulang angka yang sama dari proof.
...
[Inferensi, 11 Sep] Pembanding: dengan rubrik penilaian ide yang dipakai untuk ke-87 BUIDL (bobot ganda untuk kedalaman Attestcoin), Tinjau mendapat 25/30 dan berada di posisi 5. Titik terlemahnya adalah manfaat dunia nyata bagi Creditcoin (belum ada konsumen) dan K1 yang hanya memakai `0x0FD2` di dalam kontrak.
